...
app.controller('PMController', ['$scope', function (scope) {
    ...
    scope.onDeleteStore = function() {
        if( scope.ark.HasStore() == false ) {
            ...
            scope.ark.DeleteStore( scope.store_id, function() {
                scope.ark.store = null;
                scope.store_id = null;
                scope.doSelectStore();
            },
            scope.errorHandler );
    ...
    scope.onNewStore = function() {
        ...
        if( store_title ) {
            scope.ark.AddStore( store_title, function() {
                scope.setError(null);
                scope.doSelectStore();
            },
            scope.errorHandler );
    ...
    scope.doLogin = function() {
        ...
        if( scope.setKey( $('#key').val() ) == true ) { 
            scope.ark.Login( username, password, function(token) {
    ...
    scope.updateSessionTime = function() {
        if( scope.ark.config != null ) {
            var token_duration_minutes = scope.ark.config.token_duration;
            var token_life = Date.now() - scope.ark.token_time;
            ...
            $('#session_time').html( "Session started " + $.timeago(scope.ark.token_time) );
        }
    };
}]);
